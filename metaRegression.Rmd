...
1. $$\log \left( \frac{y_i}{t_i} \right) = \beta_0 + \beta_1 x_{\text{cyclophosphamide dose}, i} + \beta_2 I_{\text{high dose taxane}, i} + \sigma_j$$
1. $$\log \left( \frac{y_i}{t_i} \right) = \beta_0 + \beta_1 x_{\text{cyclophosphamide dose}, i} + \beta_2 I_{\text{high dose taxane}, i} + \gamma x_{\text{cyclophosphamide dose}, i} I_{\text{high dose taxane}, i} + \sigma_j$$
1. $$\log \left( \frac{y_i}{t_i} \right) = \beta_0 + \beta_1 I_{\text{high dose cyclophosphamide}, i} + \beta_2 I_{\text{high dose taxane}, i} + \sigma_j$$
1. $$\log \left( \frac{y_i}{t_i} \right) = \beta_0 + \beta_1 I_{\text{high dose cyclophosphamide}, i} + \beta_2 I_{\text{high dose taxane}, i} + \gamma I_{\text{high dose cyclophosphamide}, i} I_{\text{high dose taxane}, i} + \sigma_j$$
...
metareg <- function (D) {
  ...
  MLin <- rma.mv(yi ~ xCyc + isDocetaxel + isPaclitaxel,
                 vi,
                 random = randomEffect,
                 data = D)
  MLinInt <- rma.mv(yi ~ xCyc + isDocetaxel + isPaclitaxel + xCyc * isPaclitaxel,
                    vi,
                    random = randomEffect,
                    data = D)
  MBin <- rma.mv(yi ~ isCycHighDose + isTaxDoseHigh,
                 vi,
                 random = randomEffect,
                 data = D)
  MBinInt <- rma.mv(yi ~ isCycHighDose + isTaxDoseHigh + isCycHighDose * isTaxDoseHigh,
                    vi,
                    random = randomEffect,
                    data = D)
  list(rmaLin = MLin,
...
plotreg <- function (M, D, title) {
  ...
  X0 <- data.table(x1 = seq(min(X[isDocetaxelTRUE == 0 & isPaclitaxelTRUE == 0, xCyc]),
                            max(X[isDocetaxelTRUE == 0 & isPaclitaxelTRUE == 0, xCyc]),
                            length.out = 100),
                   x2 = rep(0, 100),
                   x3 = rep(0, 100))
  X1 <- data.table(x1 = seq(min(X[isDocetaxelTRUE == 1 & isPaclitaxelTRUE == 0, xCyc]),
                            max(X[isDocetaxelTRUE == 1 & isPaclitaxelTRUE == 0, xCyc]),
                            length.out = 100),
                   x2 = rep(1, 100),
                   x3 = rep(0, 100))
  X2 <- data.table(x1 = seq(min(X[isDocetaxelTRUE == 0 & isPaclitaxelTRUE == 1, xCyc]),
                            max(X[isDocetaxelTRUE == 0 & isPaclitaxelTRUE == 1, xCyc]),
                            length.out = 100),
                   x2 = rep(0, 100),
                   x3 = rep(1, 100))
  X <- rbindlist(list(X0, X1, X2))
  names(X) <- varnames
  ...
  keyvar <- c("malType", "xCyc", "isDocetaxelTRUE", "isPaclitaxelTRUE")
  setkeyv(yhat, keyvar)
  ...
  D <- D[isDocetaxel == FALSE & isPaclitaxel == FALSE, color := "A"]
  D <- D[isDocetaxel == FALSE & isPaclitaxel == TRUE , color := "B"]
  D <- D[isDocetaxel == TRUE  & isPaclitaxel == FALSE, color := "C"]
...
* Crude statistics
  ...
  * Total number of `r mal`: `r D3[, format(sum(malN), big.mark = ",")]`
...
* High dose taxane confounded the cyclophosphamide dose response
  * High dose taxane increased `r mal` rate by `r sprintf("%.03g", exp(M$rmaLin$b[which(row.names(M$rmaLin$b) == "isTaxDoseHighTRUE")]))` times (95% CI: `r sprintf("%.03g", exp(M$rmaLin$ci.lb[which(row.names(M$rmaLin$b) == "isTaxDoseHighTRUE")]))`, `r sprintf("%.03g", exp(M$rmaLin$ci.ub[which(row.names(M$rmaLin$b) == "isTaxDoseHighTRUE")]))`; p = `r sprintf("%.03g", M$rmaLin$pval[which(row.names(M$rmaLin$b) == "isTaxDoseHighTRUE")])`)
* High dose taxane did not modify the dose response effect of cyclophosphamide
  * Interaction term estimate was `r sprintf("%.03g", M$rmaLinInt$b[which(row.names(M$rmaLinInt$b) == "xCyc:isTaxDoseHighTRUE")])` (95% CI: `r sprintf("%.03g", M$rmaLinInt$ci.lb[which(row.names(M$rmaLinInt$b) == "xCyc:isTaxDoseHighTRUE")])`, `r sprintf("%.03g", M$rmaLinInt$ci.ub[which(row.names(M$rmaLinInt$b) == "xCyc:isTaxDoseHighTRUE")])`; p = `r sprintf("%.03g", M$rmaLinInt$pval[which(row.names(M$rmaLinInt$b) == "xCyc:isTaxDoseHighTRUE")])`)
