...
D2 <- drop_na(D2, xCyc, isCycHighDose, isTaxDoseHigh)
```
...
metareg <- function (D) {
  require(metafor)
  xData <- unique(D[, .(xCyc, isCycHighDose, isTaxDoseHigh, malType)])
  D <- escalc("IRLN", xi = malN, ti = py, data = D)
  randomEffect <- list(~ 1 | id)
  MLin <- rma.mv(yi ~ xCyc + isTaxDoseHigh,
                 vi,
                 random = randomEffect,
                 data = D)
  MLinInt <- rma.mv(yi ~ xCyc + isTaxDoseHigh + xCyc * isTaxDoseHigh,
                    vi,
                    random = randomEffect,
                    data = D)
...
plotreg <- function (M, D, title) {
  ...
  X0 <- data.table(x1 = seq(min(X[isTaxDoseHighTRUE == 0, xCyc]), max(X[isTaxDoseHighTRUE == 0, xCyc]),
                            length.out = 100),
                   x2 = rep(0, 100))
  X1 <- data.table(x1 = seq(min(X[isTaxDoseHighTRUE == 1, xCyc]), max(X[isTaxDoseHighTRUE == 1, xCyc]),
                            length.out = 100),
                   x2 = rep(1, 100))
  X <- rbind(X0, X1)
  names(X) <- varnames
  ...
  keyvar <- c("malType", "xCyc", "isTaxDoseHighTRUE")
  setkeyv(yhat, keyvar)
  setkeyv(ci.lb, keyvar)
  setkeyv(ci.ub, keyvar)
  yhat <- merge(merge(yhat, ci.lb), ci.ub)
  pvalues <- c(M$pval[which(row.names(M$b) == "isTaxDoseHighTRUE")])
  ...
  G <- G + geom_point(alpha = 1/2,
                      position = "jitter")
  G <- G + geom_line(data = yhat[isTaxDoseHighTRUE == 0, ],
                     aes(x = xCyc * xscale, y = pred),
                     inherit.aes = FALSE,
                     color = pal[1])
  G <- G + geom_line(data = yhat[isTaxDoseHighTRUE == 1, ],
                     aes(x = xCyc * xscale, y = pred),
                     inherit.aes = FALSE,
                     color = pal[2])
  G <- G + geom_ribbon(data = yhat[isTaxDoseHighTRUE == 0, ],
                       aes(x = xCyc * xscale, ymax = ci.ub, ymin = ci.lb),
                       inherit.aes = FALSE,
                       fill = pal[1],
                       alpha = 1/8)
  G <- G + geom_ribbon(data = yhat[isTaxDoseHighTRUE == 1, ],
                       aes(x = xCyc * xscale, ymax = ci.ub, ymin = ci.lb),
                       inherit.aes = FALSE,
                       fill = pal[2],
                       alpha = 1/8)
